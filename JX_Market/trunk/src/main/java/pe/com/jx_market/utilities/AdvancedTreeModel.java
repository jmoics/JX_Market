...
public class AdvancedTreeModel extends DefaultTreeModel<DTO_Categoria> {
	private static final long serialVersionUID = -5513180500300189445L;
	
	DefaultTreeNode<DTO_Categoria> _root;

	public AdvancedTreeModel(CategoriaTreeNode<DTO_Categoria> contactTreeNode) {
	 ...
	 */
	public void remove(DefaultTreeNode<DTO_Categoria> parent, int indexFrom, int indexTo) throws IndexOutOfBoundsException {
	...
	public void remove(DefaultTreeNode<DTO_Categoria> target) throws IndexOutOfBoundsException {
		int index = 0;
	 ...
	 *             - indexFrom < 0 or indexTo > number of parent's children
	 */
	public void insert(DefaultTreeNode<DTO_Categoria> parent, int indexFrom, int indexTo, DefaultTreeNode<DTO_Categoria>[] newNodes)
			throws IndexOutOfBoundsException {
		DefaultTreeNode<DTO_Categoria> stn = parent;
	 ...
	 */
	public void add(DefaultTreeNode<DTO_Categoria> parent, DefaultTreeNode<DTO_Categoria>[] newNodes) {
	...
	private DefaultTreeNode<DTO_Categoria> dfSearchParent(DefaultTreeNode<DTO_Categoria> node, DefaultTreeNode<DTO_Categoria> target) {
		if (node.getChildren() != null && node.getChildren().contains(target)) {
			return node;
		} else {
			int size = getChildCount(node);
			for (int i = 0; i < size; i++) {
				DefaultTreeNode<DTO_Categoria> parent = dfSearchParent((DefaultTreeNode<DTO_Categoria>) getChild(node, i), target);
				if (parent != null) {
					return parent;
				}
			}
